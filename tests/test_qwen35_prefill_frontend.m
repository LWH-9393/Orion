// tests/test_qwen35_prefill_frontend.m — Qwen3.5 ANE frontend smoke tests
...
static const OrionModelConfig kQwen35 = {
    .n_layer = 24,
    .n_head = 8,
    .n_kv_head = 2,
    .d_model = 1024,
...
static bool test_q_proj(void) {
    OrionGraph* g = orion_frontend_qwen35_prefill_q_proj(3, 64, &kQwen35);
    int expected = 2 * kQwen35.n_head * kQwen35.head_dim;
    ...
    if (!mil) {
        NSLog(@"q_proj compile_graph returned nil");
        return false;
    }
...
static bool test_kv_proj(void) {
    OrionGraph* g = orion_frontend_qwen35_prefill_kv_proj(3, 64, &kQwen35);
    ...
    NSString* mil = compile_graph(g);
    if (!mil) {
        NSLog(@"kv_proj compile_graph returned nil");
        return false;
    }
...
static bool test_ffn(void) {
    OrionGraph* g = orion_frontend_qwen35_prefill_ffn(3, 64, &kQwen35);
    NSString* mil = compile_graph(g);
    if (!mil) {
        NSLog(@"ffn compile_graph returned nil");
        return false;
    }
...
int main(int argc __attribute__((unused)), char* argv[] __attribute__((unused))) {
    @autoreleasepool {
        printf("test_qwen35_prefill_frontend:\n");
        TEST(q_proj);
        TEST(kv_proj);
        TEST(ffn);
